...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    await supabase.from('team_members').insert([{ team_id: teamId, user_id: recipientId, role: 'member', status: 'pending' }]);

    // Notify recipient
    await supabase.from('notifications').insert([{ user_id: recipientId, message: `You've been invited to join team: ${team.name}` }]);
