...
export default function HackathonsPage() {
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      try {
        const [devRes, customRes] = await Promise.all([
          fetch('/api/hackathons/devfolio'),
          fetch('/api/admin/hackathons')
        ]);

        if (devRes.ok) {
          const devData = await devRes.json();
          setDevfolioHackathons(devData.hits || []);
        }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 animate-fade-in-up">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
        ...
        <div>
          ...
          <div className="space-y-8">
            {sortedCustom.length === 0 ? (
              <p className="text-xl font-bold text-gray-600 bg-white p-6 border-4 border-black border-dashed">
        ...
        {/* Devfolio Hackathons Column */}
        <div>
          <h2 className="text-3xl font-black uppercase mb-8 flex items-center bg-white inline-block px-4 py-2 border-4 border-black shadow-[4px_4px_0_0_#000]">
            <ExternalLink className="mr-3 h-8 w-8 text-cyan-500" /> Devfolio Radar
          </h2>

          <div className="space-y-8">
            {devfolioHackathons.length === 0 ? (
              <p className="text-xl font-bold text-gray-600 bg-white p-6 border-4 border-black border-dashed">
                No external hackathons found.
              </p>
            ) : (
              devfolioHackathons.map(hack => (
                <div key={hack._id} className="bg-white border-4 border-black shadow-[6px_6px_0_0_#000] hover:shadow-[10px_10px_0_0_#000] transition-all hover:-translate-y-1">
                  <div className="p-6">
                    <h3 className="text-3xl font-black uppercase mb-3 text-cyan-600">{hack.name}</h3>
                    <p className="font-bold text-lg mb-4 line-clamp-3">{hack.desc || hack.description}</p>
                    
                    <div className="flex flex-wrap gap-4 text-sm font-black uppercase bg-gray-100 p-3 border-2 border-black inline-flex">
                      {hack.starts_at && (
                        <span className="flex items-center">
                          <Calendar className="w-4 h-4 mr-2"/> 
                          {new Date(hack.starts_at).toLocaleDateString()}
                        </span>
                      )}
                      {hack.ends_at && hack.ends_at !== hack.starts_at && (
                        <span className="flex items-center border-l-2 border-gray-400 pl-4">
                          {new Date(hack.ends_at).toLocaleDateString()}
                        </span>
                      )}
                    </div>
                  </div>
                  <div className="border-t-4 border-black p-4 bg-cyan-400 flex flex-col sm:flex-row gap-4 justify-between items-center text-center">
                    <span className="font-bold uppercase tracking-wider text-black">External Event</span>
                    <div className="flex flex-wrap gap-2 justify-center">
                      <Link href={`/teams`} className="brutal-btn bg-lime-400 text-black px-4 py-2 flex items-center shadow-[2px_2px_0_0_#000]">
                        Join Team
                      </Link>
                      <a 
                        href={hack.url || (hack.slug ? `https://${hack.slug}.devfolio.co` : '#')} 
                        target="_blank" 
                        rel="noopener noreferrer"
                        className="brutal-btn bg-black text-white px-4 py-2 flex items-center shadow-[2px_2px_0_0_#FFF]"
                      >
